...
getExchangeRatesEl.addEventListener("click", async () => {
  ...
  if (bool) {
    ...
    const temp = document.createTextNode("Invalid Base Currency");
    const h3Tag = document.createElement("h3");
    h3Tag.appendChild(temp);
    exchangeResEl.appendChild(h3Tag);
  }
});
...
button.addEventListener("click", async () => {
  try {
    ...
    if (fromCurrencyValue === undefined || toCurrencyValue === undefined) {
      throw new Error("Currency Doesn't Exist");
    }
    ...
    h2Element.textContent = error.message;
    exchangeResultDivEl.appendChild(h2Element);
  }
});
